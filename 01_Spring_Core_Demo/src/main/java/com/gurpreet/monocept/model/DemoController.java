...
//	@Autowired
//	public DemoController(ICoach coach) { // constructor injection
//		this.myCoach = coach;
//	}
//
//	@Autowired
...
//	}
	...
	public DemoController(@Qualifier("cricketCoach") ICoach coach,
			@Qualifier("cricketCoach") ICoach secondCoach) { // Qualifier for dynamic injection
		this.myCoach = coach;
		this.mySecondCoach = secondCoach;
		System.out.println("Inside - " + getClass().getSimpleName());
	}
	
	@GetMapping("/check")
	public String checkScopes() {
		return "check myCoach == mySecondCoach : " + (myCoach == mySecondCoach);
	}

	@GetMapping("/workoutDetails")
	public String getworkoutDetailsByCoach() {
